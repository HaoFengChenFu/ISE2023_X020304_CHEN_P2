...
{
	if(sel_SNTP_Server == 1){			// Para el servidor 1: Instituto IMDEA Software
		if (netSNTPc_GetTime((NET_ADDR *)&ntp_server_1, time_callback) == netOK) {					// Antes:   (netSNTPc_GetTime(NULL, time_callback) == netOK)
			printf ("SNTP request sent to Instituto IMDEA Software .\n");
		}
		else {
		...
		if (netSNTPc_GetTime((NET_ADDR *)&ntp_server_2, time_callback) == netOK) {
			printf ("SNTP request sent to RedIris.\n");
		}
		else {
			printf ("SNTP not ready or bad parameters.\n");
		}
