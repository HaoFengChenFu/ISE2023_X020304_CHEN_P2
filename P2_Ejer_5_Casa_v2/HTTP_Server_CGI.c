...
void netCGI_ProcessData (uint8_t code, const char *data, uint32_t len) {
  ...
  do {
    ...
    if (var[0] != 0) {
      ...
      else if (strcmp (var, "ctrl=Browser") == 0) {        // Running Leds
        LEDrun = false;
      }
      
      // Añadido para cambiar de Servidor NTP *********************************************************************************************************************************************
      else if (strncmp (var, "serverSNTP=1", strlen("serverSNTP=1")) == 0) {
          printf("Servidor 1: Instituto IMDEA Software\n");
          sel_SNTP_Server = 1;
      }
      else if (strncmp (var, "serverSNTP=2", strlen("serverSNTP=2")) == 0) {
          printf("Servidor 2: RedIris\n");
          sel_SNTP_Server = 2;
      }
      ...
      else if (strncmp (var, "horaAlarma=", strlen("horaAlarma=")) == 0) {
				
//				if(atoi(&var[12]) != 0){
//					hora = atoi(&var[11]);
//				}
//				else if (atoi(&var[12]) == 0){
//					hora = atoi(&var[11]);
//				}
				horaAlarmaRecibido = atoi(&var[11]);
				printf("Hora Alarma: %d\n", horaAlarmaRecibido);
				
      }
      else if (strncmp (var, "minutoAlarma=", strlen("minutoAlarma=")) == 0) {
